...
async def execute_task_pipeline(
    ...
    try:
        ...
        # Set task context for time constraints (硬约束)
        if metadata:
            main_agent_tool_manager.set_task_context(metadata)
            if sub_agent_tool_managers:
                for sub_tool_manager in sub_agent_tool_managers.values():
                    sub_tool_manager.set_task_context(metadata)
